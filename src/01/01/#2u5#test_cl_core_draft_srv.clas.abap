CLASS /2u5/test_cl_core_draft_srv DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    METHODS create
      IMPORTING
        !draft     TYPE /2u5/test_if_types=>ty_s_draft
    ...
    METHODS read_draft
      IMPORTING
        !id           TYPE clike
      RETURNING
        VALUE(result) TYPE /2u5/test_if_core_types=>ty_s_db.

    METHODS read_info
      IMPORTING
        !id           TYPE clike
      RETURNING
        VALUE(result) TYPE /2u5/test_if_types=>ty_s_draft.
...
CLASS /2u5/test_cl_core_draft_srv IMPLEMENTATION.


  METHOD cleanup.

    DATA(lv_four_hours_ago) = /2u5/test_cl_util=>time_substract_seconds(
        time    = /2u5/test_cl_util=>time_get_timestampl( )
        seconds = 60 * 60 * 4 ).

    DELETE FROM /2u5/test_t_01 WHERE timestampl < @lv_four_hours_ago.
    COMMIT WORK.
  ...
  METHOD create.
    ...
    DATA(ls_db) = VALUE /2u5/test_if_core_types=>ty_s_db(
        id                = draft-id
        id_prev           = draft-id_prev
        id_prev_app       = draft-id_prev_app
        id_prev_app_stack = draft-id_prev_app_stack
        uname             = /2u5/test_cl_util=>context_get_user_tech( )
        timestampl        = /2u5/test_cl_util=>time_get_timestampl( )
        data              = model_xml ).

    MODIFY /2u5/test_t_01 FROM @ls_db.
    IF sy-subrc <> 0.
