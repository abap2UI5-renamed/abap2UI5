CLASS z2ui6_cl_core_srv_draft DEFINITION
  ...
  PUBLIC SECTION.
    ...
    METHODS create
      IMPORTING
        draft     TYPE z2ui6_if_types=>ty_s_draft
    ...
    METHODS read_draft
      IMPORTING
        !id           TYPE clike
      RETURNING
        VALUE(result) TYPE z2ui6_if_core_types=>ty_s_db.

    METHODS read_info
      IMPORTING
        !id           TYPE clike
      RETURNING
        VALUE(result) TYPE z2ui6_if_types=>ty_s_draft.
...
CLASS z2ui6_cl_core_srv_draft IMPLEMENTATION.
  METHOD cleanup.
    ...
    DATA(lv_four_hours_ago) = z2ui6_cl_util=>time_substract_seconds( time    = z2ui6_cl_util=>time_get_timestampl( )
                                                                     seconds = 60 * 60 * 4 ).

    DELETE FROM z2ui6_t_01 WHERE timestampl < @lv_four_hours_ago.
    COMMIT WORK.
  ...
  METHOD create.
    ...
    DATA(ls_db) = VALUE z2ui6_if_core_types=>ty_s_db( id                = draft-id
                                                      id_prev           = draft-id_prev
                                                      id_prev_app       = draft-id_prev_app
                                                      id_prev_app_stack = draft-id_prev_app_stack
                                                      uname             = z2ui6_cl_util=>context_get_user_tech( )
                                                      timestampl        = z2ui6_cl_util=>time_get_timestampl( )
                                                      data              = model_xml ).

    MODIFY z2ui6_t_01 FROM @ls_db.
    IF sy-subrc <> 0.
