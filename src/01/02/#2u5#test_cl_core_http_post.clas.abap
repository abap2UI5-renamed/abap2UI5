CLASS /2u5/test_cl_core_http_post DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    DATA ms_request      TYPE /2u5/test_if_core_types=>ty_s_http_request_post.
    DATA ms_response     TYPE /2u5/test_if_core_types=>ty_s_http_response_post.
...
CLASS /2u5/test_cl_core_http_post IMPLEMENTATION.


  METHOD constructor.

    mv_request_json = val.
    mo_action = NEW /2u5/test_cl_core_action( me ).
  ...
  METHOD main_end.
    ...
    ms_response = VALUE #(
        ...
        s_front-app    = /2u5/test_cl_util=>rtti_get_classname_by_ref( mo_action->mo_app->mo_app )
        ).
  ...
  METHOD main_process.
    TRY.
        DATA li_client TYPE REF TO /2u5/test_cl_core_client.
        TRY.
            CREATE OBJECT li_client TYPE ('ZCL_2UI5_CUSTOM_CLIENT')
                EXPORTING
                    action = mo_action.
          CATCH cx_root.
            li_client = NEW /2u5/test_cl_core_client( mo_action ).
        ENDTRY.
