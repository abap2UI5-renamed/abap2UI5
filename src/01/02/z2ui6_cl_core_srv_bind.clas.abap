CLASS z2ui6_cl_core_srv_bind DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    DATA mr_attri  TYPE REF TO z2ui6_if_core_types=>ty_s_attri .
    DATA ms_config TYPE z2ui6_if_core_types=>ty_s_bind_config .
    DATA mv_type   TYPE string .

    METHODS constructor
      IMPORTING
        !app TYPE REF TO z2ui6_cl_core_app .

    METHODS main_local
      IMPORTING
        !val          TYPE data
        !config       TYPE z2ui6_if_core_types=>ty_s_bind_config OPTIONAL
      RETURNING
...
CLASS z2ui6_cl_core_srv_bind IMPLEMENTATION.
  ...
  METHOD bind_tab_cell.
    ...
    DATA(lt_attri) = z2ui6_cl_util=>rtti_get_t_attri_by_any( ms_config-tab ).
  ...
  METHOD check_raise_existing.
    ...
    IF mr_attri->custom_mapper IS BOUND.
      ...
      DATA(lv_name1) = z2ui6_cl_util=>rtti_get_classname_by_ref( mr_attri->custom_mapper ).
  ...
  METHOD get_client_name.
    ...
    result = COND #( WHEN mv_type = z2ui6_if_core_types=>cs_bind_type-two_way
        THEN `/` && z2ui6_if_core_types=>cs_ui5-two_way_model )
        && `/` && result.
  ...
  METHOD main.
    ...
    IF z2ui6_cl_util=>check_bound_a_not_inital( config-tab ).
  ...
  METHOD main_cell.
    ...
    DATA(lo_bind) = NEW z2ui6_cl_core_srv_bind( mo_app ).
  ...
  METHOD main_local.
    TRY.
        ...
        DATA(lo_json) = CAST z2ui6_if_ajson( z2ui6_cl_ajson=>new( ) ).
        lo_json->set( iv_path = `/` iv_val = val ).

        IF config-custom_mapper IS BOUND.
          lo_json = lo_json->map( config-custom_mapper ).
        ELSE.
          lo_json = lo_json->map( z2ui6_cl_ajson_mapping=>create_upper_case( ) ).
        ENDIF.

        IF config-custom_filter IS BOUND.
          lo_json = lo_json->filter( config-custom_filter ).
        ELSE.
          lo_json = lo_json->filter( z2ui6_cl_ajson_filter_lib=>create_empty_filter( ) ).
        ENDIF.

        DATA(lv_id) = to_upper( z2ui6_cl_util=>uuid_get_c22( ) ).
        INSERT VALUE #( name_client     = |/{ lv_id }|
                        name            = lv_id
                        json_bind_local = lo_json
                        bind_type       = z2ui6_if_core_types=>cs_bind_type-one_time )
  ...
  METHOD update_model_attri.
    ...
    mr_attri->view          = COND #( WHEN ms_config-view IS INITIAL THEN z2ui6_if_client=>cs_view-main ELSE ms_config-view ).
