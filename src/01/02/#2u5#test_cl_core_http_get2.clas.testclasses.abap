...
CLASS /2u5/test_cl_core_http_get2 DEFINITION LOCAL FRIENDS ltcl_test_http_get.

CLASS ltcl_test_http_get IMPLEMENTATION.

  METHOD file_not_initial.

    DATA(lo_get) = NEW /2u5/test_cl_core_http_get2( ).
...
*    ENDIF.
  ...
  METHOD debugging_tools.
    ...
    DATA(lv_index_html) = to_upper( /2u5/test_cl_cc_debug_tool=>get_js( ) ) ##NEEDED.
    IF lv_index_html CS `<`.
