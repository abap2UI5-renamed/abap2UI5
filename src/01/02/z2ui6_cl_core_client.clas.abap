CLASS z2ui6_cl_core_client DEFINITION
  PUBLIC
  CREATE PUBLIC.
  ...
  PUBLIC SECTION.
    ...
    DATA mo_action TYPE REF TO z2ui6_cl_core_action.

    METHODS constructor
      IMPORTING
        action TYPE REF TO z2ui6_cl_core_action.
...
CLASS z2ui6_cl_core_client IMPLEMENTATION.
...
*    mo_action->ms_next-s_set-s_follow_up_action-custom_js = val.
  ...
  METHOD z2ui6_if_client~get_app.
    ...
    IF id IS NOT INITIAL.
      DATA(lo_app) = z2ui6_cl_core_app=>db_load( id ).
  ...
  METHOD z2ui6_if_client~message_box_display.
    ...
    IF z2ui6_cl_util=>rtti_check_clike( text ) = abap_false.
      DATA(lt_msg) = z2ui6_cl_util=>msg_get( text ).
      IF lines( lt_msg ) = 1.
        DATA(lv_text) = lt_msg[ 1 ]-text.

        DATA(lv_type) = z2ui6_cl_util=>ui5_get_msg_type( lt_msg[ 1 ]-type ).
      ...
      ELSEIF lines( lt_msg ) > 1.
        ...
        IF title IS INITIAL.
          lv_title = SWITCH #( lt_msg[ 1 ]-type
                               ...
                               ELSE          `Information` ).
        ENDIF.
        lv_type = z2ui6_cl_util=>ui5_get_msg_type( lt_msg[ 1 ]-type ).
      ELSE.
        RETURN.
      ENDIF.
    ELSE.
  ...
  METHOD z2ui6_if_client~nav_app_call.
    ...
    IF app IS NOT BOUND.
      z2ui6_cl_util=>x_raise( `NAV_APP_LEAVE_TO_INITIAL_APP_ERROR` ).
    ENDIF.

    mo_action->ms_next-o_app_call = app.

    IF app->id_app IS INITIAL.
      app->id_app = z2ui6_cl_util=>uuid_get_c32( ).
    ENDIF.
    result = app->id_app.
  ENDMETHOD.
  ...
  METHOD z2ui6_if_client~nav_app_leave.
    ...
    IF app IS NOT SUPPLIED.
      app = z2ui6_if_client~get_app( z2ui6_if_client~get( )-s_draft-id_prev_app_stack ).
    ENDIF.
  ...
  METHOD z2ui6_if_client~_bind.
    ...
    DATA(lo_bind) = NEW z2ui6_cl_core_srv_bind( mo_action->mo_app ).
    result = lo_bind->main( val    = z2ui6_cl_util=>conv_get_as_data_ref( val )
                            type   = z2ui6_if_core_types=>cs_bind_type-one_way
  ...
  METHOD z2ui6_if_client~_bind_edit.
    ...
    DATA(lo_bind) = NEW z2ui6_cl_core_srv_bind( mo_action->mo_app ).
    result = lo_bind->main( val    = z2ui6_cl_util=>conv_get_as_data_ref( val )
                            type   = z2ui6_if_core_types=>cs_bind_type-two_way
                            config = VALUE #( path_only          = path
                                              custom_filter      = custom_filter
  ...
  METHOD z2ui6_if_client~_bind_local.
  ...
  METHOD z2ui6_if_client~_event.
    ...
    DATA(lo_ui5) = NEW z2ui6_cl_core_srv_event( ).
    ...
    IF r_data IS NOT INITIAL.
      ...
      mo_action->ms_next-r_data = z2ui6_cl_util=>conv_copy_ref_data( r_data ).
    ENDIF.
  ...
  METHOD z2ui6_if_client~set_session_stateful.
    ...
    ELSE.
      ...
      CAST z2ui6_if_app( mo_action->mo_app->mo_app )->check_sticky = abap_false.
    ENDIF.
  ...
  METHOD z2ui6_if_client~check_app_prev_stack.
    ...
    DATA(ls_get) = z2ui6_if_client~get( ).
  ...
  METHOD z2ui6_if_client~check_on_init.
    ...
    result = xsdbool( CAST z2ui6_if_app(  mo_action->mo_app->mo_app )->check_initialized = abap_false ).
  ...
  METHOD z2ui6_if_client~get_app_prev.
    ...
    DATA(ls_get) = z2ui6_if_client~get( ).
    result = z2ui6_if_client~get_app( ls_get-s_draft-id_prev_app ).
