CLASS /2ui5/test_cl_core_app_info DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
*    DATA mv_device_gav TYPE string.
    ...
    CLASS-METHODS factory
      RETURNING
        VALUE(result) TYPE REF TO /2ui5/test_cl_core_app_info.
...
CLASS /2ui5/test_cl_core_app_info IMPLEMENTATION.
  ...
  METHOD view_display_start.
    ...
    DATA(page2) = /2ui5/test_cl_xml_view=>factory_popup(
         )->dialog(
...
*   simple_form2->text( client->_bind( mv_device_type ) ).
    ...
    simple_form2->checkbox( enabled = abap_false selected = /2ui5/test_cl_util=>context_check_abap_cloud( ) ).

    DATA(lv_count) = CONV string( NEW /2ui5/test_cl_core_draft_srv( )->count_entries( ) ).
    simple_form2->toolbar( )->title( `abap2UI5` ).
    simple_form2->label( `Version ` ).
    simple_form2->text( /2ui5/test_if_app=>version ).
