CLASS z2ui6_cl_util DEFINITION
  PUBLIC
  CREATE PUBLIC
  INHERITING FROM z2ui6_cl_abap_api.
...
*  license: MIT.
    ...
    CLASS-METHODS rtti_get_t_ddic_fixed_values
      ...
      RETURNING
        VALUE(result) TYPE z2ui6_cl_abap_api=>ty_t_fix_val ##NEEDED.
...
CLASS z2ui6_cl_util IMPLEMENTATION.
  ...
  METHOD filter_update_tokens.
    ...
    DATA(lt_range) = z2ui6_cl_util=>filter_get_range_t_by_token_t( result[ name = name ]-t_token ).
  ...
  METHOD json_parse.
    TRY.
        ...
        z2ui6_cl_ajson=>parse( val )->to_abap(
          EXPORTING
  ...
  METHOD json_stringify.
    TRY.
        ...
        DATA(li_ajson) = CAST z2ui6_if_ajson( z2ui6_cl_ajson=>create_empty( ) ).
...
`x, nsis, objectivec, ocaml, pascal, perl, pgsql, php, plain_text, powershell, praat, prolog, properties, protobuf, python, r, razor, rdoc, rhtml, rst, ruby, rust, sass, scad, scala, scheme, scss, sh, sjs, smarty, snippets, soy_template, space, sql,` &&
  ...
  METHOD x_check_raise.
    ...
    IF when = abap_true.
      RAISE EXCEPTION TYPE z2ui6_cx_util_error EXPORTING val = v.
    ENDIF.
