CLASS /2u5/test_cl_util DEFINITION
  PUBLIC
  CREATE PUBLIC
  INHERITING FROM /2u5/test_cl_abap_api.
...
*  license: MIT.
    ...
    CLASS-METHODS rtti_get_t_ddic_fixed_values
      ...
      RETURNING
        VALUE(result) TYPE /2u5/test_cl_abap_api=>ty_t_fix_val ##NEEDED.
    ...
    CLASS-METHODS url_param_create_url
      IMPORTING
        !t_params     TYPE /2u5/test_if_types=>ty_t_name_value
      RETURNING
        VALUE(result) TYPE string.
    ...
    CLASS-METHODS url_param_get_tab
      IMPORTING
        i_val            TYPE clike
      RETURNING
        VALUE(rt_params) TYPE /2u5/test_if_types=>ty_t_name_value.
    ...
    CLASS-METHODS filter_get_token_range_mapping
      RETURNING
        VALUE(result) TYPE /2u5/test_if_types=>ty_t_name_value.
...
CLASS /2u5/test_cl_util IMPLEMENTATION.


  METHOD db_delete_by_handle.

    DELETE FROM /2u5/test_t_02
        WHERE
           uname = @uname
            AND handle = @handle
            AND handle2 = @handle2
  ...
  METHOD db_load_by_handle.
    ...
    DATA lt_db TYPE STANDARD TABLE OF /2u5/test_t_02 WITH EMPTY KEY.
  ...
  METHOD db_save.
    ...
    DATA(ls_db) = VALUE /2u5/test_t_02(
        uname   = uname
        handle  = handle
        handle2 = handle2
        handle3 = handle3
    ...
    MODIFY /2u5/test_t_02 FROM @ls_db.
    ASSERT sy-subrc = 0.
  ...
  METHOD json_parse.
    TRY.
        ...
        /2u5/test_cl_ajson=>parse( val )->to_abap(
          EXPORTING
            iv_corresponding = abap_true
          IMPORTING
            ev_container = data ).

      CATCH /2u5/test_cx_ajson_error INTO DATA(x).
  ...
  METHOD json_stringify.
    TRY.
        ...
        DATA(li_ajson) = CAST /2u5/test_if_ajson( /2u5/test_cl_ajson=>create_empty( ) ).
...
`x, nsis, objectivec, ocaml, pascal, perl, pgsql, php, plain_text, powershell, praat, prolog, properties, protobuf, python, r, razor, rdoc, rhtml, rst, ruby, rust, sass, scad, scala, scheme, scss, sh, sjs, smarty, snippets, soy_template, space, sql,` &&
  ...
  METHOD x_check_raise.
    ...
    IF when = abap_true.
      RAISE EXCEPTION TYPE /2u5/test_cx_util_error EXPORTING val = v.
    ENDIF.
