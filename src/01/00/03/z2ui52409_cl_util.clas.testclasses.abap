...
CLASS ltcl_test IMPLEMENTATION.
  ...
  METHOD test_db_handle.
    ...
    DATA(lv_id) = z2ui52409_cl_util=>db_save(
        uname   = `name`
        handle  = `handle1`
        handle2 = `handle2`
        handle3 = `handle3`
    ...
    z2ui52409_cl_util=>db_load_by_id(
      EXPORTING
        id     = lv_id
      IMPORTING
    ...
    CLEAR ls_row_result.
    z2ui52409_cl_util=>db_load_by_handle(
      EXPORTING
        uname   = `name`
        handle  = `handle1`
        handle2 = `handle2`
  ...
  METHOD test_db_handle_read_id.
    ...
    DATA(lv_id2) = z2ui52409_cl_util=>db_save(
        uname   = `name`
        handle  = `handle1`
        handle2 = `handle2`
        handle3 = `handle3`
...
CLASS ltcl_unit_test_abap_api IMPLEMENTATION.
  ...
  METHOD test_raise_error.
    ...
    TRY.
        IF 1 = 1.
          RAISE EXCEPTION TYPE z2ui52409_cx_util_error.
        ENDIF.
        cl_abap_unit_assert=>fail( ).

      CATCH z2ui52409_cx_util_error INTO DATA(lx).
...
CLASS ltcl_unit_test IMPLEMENTATION.
  ...
  METHOD test_boolean_check.
    ...
    cl_abap_unit_assert=>assert_equals(
        act = z2ui52409_cl_util=>boolean_check_by_data( lv_bool )
        exp = abap_true ).
    ...
    cl_abap_unit_assert=>assert_equals(
        act = z2ui52409_cl_util=>boolean_check_by_data( abap_true )
        exp = abap_true ).

    cl_abap_unit_assert=>assert_equals(
        act = z2ui52409_cl_util=>boolean_check_by_data( abap_false )
        exp = abap_true ).
  ...
  METHOD test_sql_get_by_string.
    ...
    DATA(lv_test) = ``.
    DATA(ls_sql) = z2ui52409_cl_util=>filter_get_sql_by_sql_string( lv_test ) ##NEEDED.

  ENDMETHOD.

  METHOD test_create.

    DATA(lo_test) = NEW z2ui52409_cl_util( ) ##NEEDED.

  ENDMETHOD.

  METHOD test_rtti_get_classname_by_ref.

    DATA(lo_test) = NEW z2ui52409_cl_util( ).
    DATA(lv_name) = z2ui52409_cl_util=>rtti_get_classname_by_ref( lo_test ).
    cl_abap_unit_assert=>assert_equals( exp = `Z2UI5_CL_UTIL`
                                        act = lv_name ).

    DATA(lo_test2) = NEW ltcl_test_app( ).
    DATA(lv_name2) = z2ui52409_cl_util=>rtti_get_classname_by_ref( lo_test2 ).
  ...
  METHOD test_check_bound_a_not_inital.
    ...
    cl_abap_unit_assert=>assert_true( z2ui52409_cl_util=>check_bound_a_not_inital( lr_test ) ).
    CLEAR lv_test.
    cl_abap_unit_assert=>assert_false( z2ui52409_cl_util=>check_bound_a_not_inital( lr_test ) ).
    CLEAR lr_test.
    cl_abap_unit_assert=>assert_false( z2ui52409_cl_util=>check_bound_a_not_inital( lr_test ) ).
  ...
  METHOD test_check_unassign_inital.
    ...
    cl_abap_unit_assert=>assert_false( z2ui52409_cl_util=>check_unassign_inital( lr_test ) ).

    CLEAR lv_test.
    cl_abap_unit_assert=>assert_true( z2ui52409_cl_util=>check_unassign_inital( lr_test ) ).
  ...
  METHOD rtti_check_ref_data.
    ...
    cl_abap_unit_assert=>assert_equals(
      act = z2ui52409_cl_util=>rtti_check_ref_data( lr_data )
      exp = abap_true ).

    cl_abap_unit_assert=>assert_equals(
      act = z2ui52409_cl_util=>rtti_check_ref_data( lv_test )
      exp = abap_false ).
  ...
  METHOD conv_copy_ref_data.
    ...
    DATA(lr_test2) = z2ui52409_cl_util=>conv_copy_ref_data( lr_data ).
  ...
  METHOD test_boolean_abap_2_json.
    ...
    cl_abap_unit_assert=>assert_equals(
       act = z2ui52409_cl_util=>boolean_abap_2_json( `{ABCD}` )
       exp = `{ABCD}` ).
  ...
  METHOD test_time_get_timestampl.
    ...
    DATA(lv_time) = z2ui52409_cl_util=>time_get_timestampl( ).

    DATA(lv_time2) = z2ui52409_cl_util=>time_substract_seconds(
         time    = lv_time
         seconds = 60 * 60 * 4 ).
  ...
  METHOD test_time_substract_seconds.
    ...
    DATA(lv_time) = z2ui52409_cl_util=>time_get_timestampl( ).
    DATA(lv_time2) = z2ui52409_cl_util=>time_get_timestampl( ).
  ...
  METHOD test_c_trim.
    ...
    cl_abap_unit_assert=>assert_equals(
        act = z2ui52409_cl_util=>c_trim( ` JsadfHHs  ` )
        exp = `JsadfHHs` ).
  ...
  METHOD test_c_trim_lower.
    ...
    cl_abap_unit_assert=>assert_equals(
        act = z2ui52409_cl_util=>c_trim_lower( ` JsadfHHs  ` )
        exp = `jsadfhhs` ).
  ...
  METHOD test_c_trim_upper.
    ...
    cl_abap_unit_assert=>assert_equals(
        act = z2ui52409_cl_util=>c_trim_upper( ` JsadfHHs  ` )
        exp = `JSADFHHS` ).
  ...
  METHOD test_func_get_user_tech.
    ...
    cl_abap_unit_assert=>assert_equals(
      act = sy-uname
      exp = z2ui52409_cl_util=>context_get_user_tech( ) ).

    cl_abap_unit_assert=>assert_not_initial( z2ui52409_cl_util=>context_get_user_tech( ) ).
  ...
  METHOD test_x_check_raise.
    ...
    TRY.
        z2ui52409_cl_util=>x_check_raise( xsdbool( 1 = 1 ) ).
        cl_abap_unit_assert=>fail( ).
      CATCH cx_root.
    ENDTRY.

    TRY.
        z2ui52409_cl_util=>x_check_raise( xsdbool( 1 = 3 ) ).
      CATCH cx_root.
        cl_abap_unit_assert=>fail( ).
    ENDTRY.
  ...
  METHOD test_trans_json_any_2__w_struc.
    ...
    cl_abap_unit_assert=>assert_equals(
        act = z2ui52409_cl_util=>json_stringify( ls_row )
  ...
  METHOD test_url_param_create_url.
    ...
    DATA(lt_param) = z2ui52409_cl_util=>url_param_get_tab( `https://url.com/rvice_for_ui?sap-client=100&app_start=z2ui5_cl_app_hello_world` ).
    DATA(lv_url) = z2ui52409_cl_util=>url_param_create_url( lt_param ).
  ...
  METHOD test_url_param_get.
    ...
    DATA(lv_param) = z2ui52409_cl_util=>url_param_get(
        val = `app_start`
  ...
  METHOD test_url_param_set.
    ...
    DATA(lv_param) = z2ui52409_cl_util=>url_param_set(
         name  = `app_start`
  ...
  METHOD test_x_check_raise_not.
    ...
    TRY.
        z2ui52409_cl_util=>x_check_raise( xsdbool( 1 = 2 ) ).
      CATCH z2ui52409_cx_util_error.
        cl_abap_unit_assert=>fail( ).
    ENDTRY.
  ENDMETHOD.
  ...
  METHOD test_rtti_get_type_name.
    ...
    DATA(lv_name) = z2ui52409_cl_util=>rtti_get_type_name( lv_xsdbool ).
    cl_abap_unit_assert=>assert_equals(
        act = lv_name
        exp = `XSDBOOLEAN` ).
  ...
  METHOD test_rtti_get_type_kind.
    ...
    DATA(lv_type_kind) = z2ui52409_cl_util=>rtti_get_type_kind( lv_string ).
    ...
    lv_type_kind = z2ui52409_cl_util=>rtti_get_type_kind( lr_string ).
    cl_abap_unit_assert=>assert_equals(
        act = lv_type_kind
  ...
  METHOD test_rtti_check_type_kind.
    ...
    DATA lr_string TYPE REF TO string.
    cl_abap_unit_assert=>assert_equals(
        act = z2ui52409_cl_util=>rtti_check_type_kind_dref( lv_string )
        exp = abap_false ).


    CREATE DATA lr_string.
    cl_abap_unit_assert=>assert_equals(
        act = z2ui52409_cl_util=>rtti_check_type_kind_dref( lr_string )
        exp = abap_true ).
  ...
  METHOD test_rtti_get_t_attri_by_obj.
    ...
    DATA(lt_attri) = z2ui52409_cl_util=>rtti_get_t_attri_by_oref( lo_obj ).
  ...
  METHOD test_rtti_get_t_comp_by_struc.
    ...
    DATA(lt_comp) = z2ui52409_cl_util=>rtti_get_t_attri_by_any( ls_row ).
  ...
  METHOD test_trans_xml_any_2__w_obj.
    ...
    DATA(lo_obj) = NEW ltcl_test_app( ).
    DATA(lv_xml) = z2ui52409_cl_util=>xml_stringify( lo_obj ).
  ...
  METHOD test_trans_xml_any_2__w_data.
    ...
    DATA(lv_xml) = z2ui52409_cl_util=>xml_stringify( ls_row ).
  ...
  METHOD test_c_trim_horizontal_tab.
    ...
    IF z2ui52409_cl_util=>c_trim( |{ cl_abap_char_utilities=>horizontal_tab }|
                                && |JsadfHHs|
  ...
  METHOD test_get_token_t_by_r_t.
    ...
    DATA(lt_range) = VALUE z2ui52409_cl_util=>ty_t_range(
        ( sign = 'I' option = 'EQ' low = `table` high = `` )
     ).

    DATA(lt_result) = z2ui52409_cl_util=>filter_get_token_t_by_range_t( lt_range ).

    DATA(lt_exp) = VALUE z2ui52409_cl_util=>ty_t_token(
  ...
  METHOD test_rtti_get_t_attri_by_incl.
    ...
    DATA(lt_attri) = z2ui52409_cl_util=>rtti_get_t_attri_by_include( CAST #( lo_datadescr ) ).
