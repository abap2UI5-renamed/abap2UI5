CLASS z2ui52409_cl_util DEFINITION
  PUBLIC
  CREATE PUBLIC
  INHERITING FROM z2ui52409_cl_abap_api.
...
*  license: MIT.
    ...
    CLASS-METHODS rtti_get_t_ddic_fixed_values
      ...
      RETURNING
        VALUE(result) TYPE z2ui52409_cl_abap_api=>ty_t_fix_val ##NEEDED.
    ...
    CLASS-METHODS filter_get_sql_where
      IMPORTING
        val           TYPE  z2ui52409_cl_util=>ty_t_filter_multi
      RETURNING
        VALUE(result) TYPE string.
...
CLASS z2ui52409_cl_util IMPLEMENTATION.


  METHOD db_delete_by_handle.

    DELETE FROM z2ui52409_t_02
        WHERE
           uname = @uname
            AND handle = @handle
            AND handle2 = @handle2
  ...
  METHOD db_load_by_handle.
    ...
    DATA lt_db TYPE STANDARD TABLE OF z2ui52409_t_02 WITH EMPTY KEY.
  ...
  METHOD db_save.
    ...
    DATA(ls_db) = VALUE z2ui52409_t_02(
        uname   = uname
        handle  = handle
        handle2 = handle2
        handle3 = handle3
    ...
    MODIFY z2ui52409_t_02 FROM @ls_db.
    ASSERT sy-subrc = 0.
  ...
  METHOD filter_update_tokens.
    ...
    DATA(lt_range) = z2ui52409_cl_util=>filter_get_range_t_by_token_t( result[ name = name ]-t_token ).
  ...
  METHOD json_parse.
    TRY.
        ...
        z2ui52409_cl_ajson=>parse( val )->to_abap(
          EXPORTING
  ...
  METHOD json_stringify.
    TRY.
        ...
        DATA(li_ajson) = CAST z2ui52409_if_ajson( z2ui52409_cl_ajson=>create_empty( ) ).
...
`x, nsis, objectivec, ocaml, pascal, perl, pgsql, php, plain_text, powershell, praat, prolog, properties, protobuf, python, r, razor, rdoc, rhtml, rst, ruby, rust, sass, scad, scala, scheme, scss, sh, sjs, smarty, snippets, soy_template, space, sql,` &&
  ...
  METHOD x_check_raise.
    ...
    IF when = abap_true.
      RAISE EXCEPTION TYPE z2ui52409_cx_util_error EXPORTING val = v.
    ENDIF.
