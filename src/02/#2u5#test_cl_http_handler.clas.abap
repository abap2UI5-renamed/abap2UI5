CLASS /2u5/test_cl_http_handler DEFINITION
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS factory_cloud
      ...
      RETURNING
        VALUE(result) TYPE REF TO /2u5/test_cl_http_handler.
    ...
    METHODS main
      IMPORTING
        s_config TYPE /2u5/test_if_types=>ty_s_http_config OPTIONAL.

  PROTECTED SECTION.

    CLASS-DATA so_sticky_handler TYPE REF TO /2u5/test_cl_core_http_post.
    DATA mo_server TYPE REF TO /2u5/test_cl_abap_api_http.
    DATA ms_session_attributes TYPE /2u5/test_if_types=>ty_s_http_handler_attributes.
    ...
    TYPES:
      BEGIN OF ty_s_http_res,
        ...
        t_header      TYPE /2u5/test_if_types=>ty_t_name_value,
      END OF ty_s_http_res.

    DATA ms_req TYPE ty_s_http_req.
    DATA ms_res TYPE ty_s_http_res.
    DATA ms_config TYPE /2u5/test_if_types=>ty_s_http_config.
    ...
    METHODS set_config
      IMPORTING
        is_custom_config TYPE /2u5/test_if_types=>ty_s_http_config.
...
*      ms_req_config-src     = `https://sdk.openui5.org/nightly/2/resources/sap-ui-core.js`.
  ...
  METHOD get_js_cc_startup.
    ...
    result = ` ` &&
        /2u5/test_cl_cc_timer=>get_js( ) &&
        /2u5/test_cl_cc_focus=>get_js( ) &&
        /2u5/test_cl_cc_title=>get_js( ) &&
        /2u5/test_cl_cc_lp_title=>get_js( ) &&
        /2u5/test_cl_cc_history=>get_js( ) &&
        /2u5/test_cl_cc_scrolling=>get_js( ) &&
        /2u5/test_cl_cc_info=>get_js( ) &&
        /2u5/test_cl_cc_geoloc=>get_js( ) &&
        /2u5/test_cl_cc_file_upl=>get_js( ) &&
        /2u5/test_cl_cc_multiinput=>get_js( ) &&
        /2u5/test_cl_cc_uitable=>get_js( ) &&
        /2u5/test_cl_cc_util=>get_js( ) &&
        /2u5/test_cl_cc_favicon=>get_js( ) &&
        /2u5/test_cl_cc_dirty=>get_js( ) &&
       `  `.
  ...
  METHOD factory.
    ...
    result = NEW #( ).
    result->mo_server = /2u5/test_cl_abap_api_http=>factory( server ).
  ...
  METHOD factory_cloud.
    ...
    result = NEW #( ).
    result->mo_server = /2u5/test_cl_abap_api_http=>factory_cloud(
        req = req
        res = res ).
  ...
  METHOD http_get.
    ...
    DATA(lo_app) = NEW /2u5/test_cl_core_ui5_app( ).
    ms_res-body = lo_app->index_html( ms_config ).

    NEW /2u5/test_cl_core_draft_srv( )->cleanup( ).
  ...
  METHOD http_post.
    ...
    IF so_sticky_handler IS NOT BOUND.
      DATA(lo_post) = NEW /2u5/test_cl_core_http_post( ms_req-body  ).
    ELSE.
    ...
    TRY.
        IF lo_post IS BOUND.
          DATA(li_app) = CAST /2u5/test_if_app( lo_post->mo_action->mo_app->mo_app ).
